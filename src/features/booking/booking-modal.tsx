...
export default (((props) => {
    const {doctor, isOpen, onClose} = props
    ...
    const initialFormState: BookingForm = {doctorId: doctor.id, date: '', time: '', patientName: '', patientPhone: '', reason: ''}
    const modalVariants = {
    ...
    const handleDateTimeNext = useCallback(() => {
        if (isDateTimeValid) {
            setBookingForm((prev) => ({
                ...prev, date: selectedDate, time: selectedTime
            }))
            setStep('info')
        }
            ...
            {isOpen && (
                <>
                    ...
                    >
                        {/* Modal Content */}
                        <div
                            className="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[85vh] min-h-[450px] flex flex-col overflow-hidden">
                            {/* Header - Fixed height */}
                            ...
                            <div className="flex-1 overflow-y-auto p-6">
                                ...
                                {step === 'info' && (<PatientInfoStep
                                    bookingForm={bookingForm}
                                    onFormUpdate={updateBookingForm}
                                />)}

                                {step === 'confirm' && (<ConfirmationStep doctor={doctor} bookingForm={bookingForm}/>)}
                            </div>
...
const BookingModelHeader = (({doctor, step, onClose}) => (
    <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6">
        <div className="flex items-center justify-between">
            <div>
                <h2 className="text-2xl font-bold">Đặt lịch khám</h2>
                <p className="text-blue-100 mt-1">{doctor.name} - {doctor.specialty}</p>
            </div>
            <button
                onClick={onClose}
    ...
    <div className="space-y-6">
        ...
        <div className="bg-gray-50 rounded-xl p-6 space-y-4">
            ...
            <div className="border-t pt-4 space-y-2">
                ...
                {bookingForm.reason && (<div className="flex justify-between">
        ...
        <div>
            ...
            <div className="grid grid-cols-4 gap-3">
                {timeSlots.map((slot) => (<button
                    ...
                    className={`p-3 rounded-lg font-medium transition-all duration-200 ${selectedTime === slot.time ? 'bg-blue-600 text-white shadow-lg' : slot.available ? 'bg-white text-gray-700 border border-gray-200 hover:border-blue-300 hover:bg-blue-50' : 'bg-gray-100 text-gray-400 cursor-not-allowed'}`}
                >
                    {slot.time}
                </button>))}
            </div>
