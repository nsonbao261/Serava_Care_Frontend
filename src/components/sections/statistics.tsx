                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
                        {STATS.map((stat, index) => {
                            ...
                            return (<div
                                ...
                                style={{
                                    animationDelay: `${index * 100 + 200}ms`, animationDuration: '600ms'
                                }}
                            >
                                ...
                                <div className="text-gray-600 text-sm font-medium hover:text-gray-800 transition-colors">{stat.label}</div>
                            </div>)
                        })}
                    </div>
                </GlowCard>
...
const GlowCard = ((props) => {
    const {children, className, style, glowColor = '#3b82f6', intensity = 'medium'} = props
    ...
    const intensityClasses = {low: 'before:opacity-30', medium: 'before:opacity-50', high: 'before:opacity-70'}

    return (
        <div
            ref={cardRef}
            className={cn('relative overflow-hidden rounded-lg', 'before:absolute before:inset-0 before:z-0', 'before:bg-gradient-radial before:from-current before:to-transparent', 'before:transition-opacity before:duration-300', 'before:opacity-0 hover:before:opacity-50', 'group cursor-pointer', intensityClasses[intensity], className)}
            style={{
                ...style, '--tw-gradient-from': glowColor, '--tw-gradient-to': 'transparent'
